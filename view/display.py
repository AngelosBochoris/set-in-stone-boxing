...
from view.ui_elements.battle_screen import SelectionScreen
...
class Display:
    def __init__(self):
        ...
        self.fl, self.fm, self.fs, self.ft = map(lambda x: pygame.font.SysFont(None, x, bold=True), [52, 28, 20, 16])
        ...
        self.state_drawers = {
            ...
            P1_SELECT: (lambda: self.p1_screen.draw(self.screen)),
            GAME_OVER: self._draw_game_over,
            RESOLVE: self._draw_resolve,
    ...
    def draw(self, state):
        self.state_drawers[state]()

    def _draw_main_menu(self):
        self.screen.fill(config.C_BG)
        title = self.fl.render("BOXING GAME", True, config.C_ACCENT1)
        self.screen.blit(title, title.get_rect(
            center=(config.WINDOW_W // 2, 180)))
        sub = self.fm.render("1v1 Online Multiplayer", True, config.C_SUBTEXT)
    ...
    def _draw_waiting(self):
        self.screen.fill(config.C_BG)
        text = self.fl.render("Waiting for opponent…", True, config.C_TEXT)
        ...
        dots_surf = self.fm.render(dots, True, config.C_SUBTEXT)
        self.screen.blit(dots_surf, dots_surf.get_rect(
            center=(config.WINDOW_W // 2, config.WINDOW_H // 2 + 75)))

    def _draw_connecting(self):
        self.screen.fill(config.C_BG)
        text = self.fl.render("Waiting for opponent to connect…", True, config.C_TEXT)
        self.screen.blit(text, text.get_rect(
            center=(config.WINDOW_W // 2, config.WINDOW_H // 2 - 20)))
        dots = "." * (1 + (pygame.time.get_ticks() // 500) % 3)
        dots_surf = self.fm.render(dots, True, config.C_SUBTEXT)
    ...
    def _draw_resolve(self):
        ...
        if step:
            ...
            for i, (text, colour) in enumerate(dmg_lines):
                surf = self.fs.render(text, True, colour)
                self.screen.blit(surf, surf.get_rect(
                    center=(config.WINDOW_W // 2, cy + 105 + i * 28)))
        else:
            loading = self.fm.render("Processing…", True, config.C_TEXT)
    ...
    def _draw_game_over(self):
        ...
        title = self.fl.render(text, True, colour)
        self.screen.blit(title, title.get_rect(
